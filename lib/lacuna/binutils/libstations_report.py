...
class StationsReport(lacuna.binutils.libbin.Script):
    ...
    def _res_report( self ):
        ...
        mystr += "\t-=-=-=-\n"
        mystr +=  tmpl.format(  "Food max:",     self.station.food_capacity      )
        mystr +=  tmpl.format(  "Ore max:",      self.station.ore_capacity       )
        mystr +=  tmpl.format(  "Water max:",    self.station.water_capacity     )
        mystr +=  tmpl.format(  "Energy max:",   self.station.energy_capacity    )
        return mystr
